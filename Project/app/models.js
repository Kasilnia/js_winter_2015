...
var EntryModel = function (pizza, count) {
	this.id = pizza.id
...
BasketModel.prototype.add_entry = function(pizza, count) {
	if (this.entries.hasOwnProperty(pizza.id)) {
		entry = this.entries[pizza.id];
		entry.count =  entry.count + parseInt(count);
	}
	else {
		entry = new EntryModel(pizza, count);
...
BasketModel.prototype.update_summary = function() {
	...
	for (var key in entries) {
	  if (entries.hasOwnProperty(key)) {
		count = parseInt(entries[key].count), price = parseFloat(entries[key].pizza_price);
		sum = sum + count*price;
	  }
	}
